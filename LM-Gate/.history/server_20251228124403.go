...
func OnMessage(msg ChunkMessage) error {
	...
	if msg.IsEOF {
		...
		// 3. تنظيف القطع المؤقتة بعد التجميع
		Cleanup(msg.FileID)
		return nil
	}
	...
	if s.isComplete(fileDir, total) {
		fmt.Println("🎉 اكتملت جميع القطع...")
		go s.reassemble(fileID, total)
	}
	return nil // أضف هذا السطر هنا
...
func ProcessFile(fileID string, filePath string) error {
	fmt.Printf("🚀 [SERVER] تم تجميع الملف بنجاح. بدء التحليل: %s\n", filePath)

	// هذا السطر هو الرابط مع مجلد analysis
...
func Cleanup(fileID string) {
	// تم تحديثه ليحذف المجلد المادي من القرص
