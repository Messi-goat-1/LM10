...
var _ interfaces.Item = (*List)(nil)
...
// أضف هذه الدوال بجانب الدوال الحالية (LLen, LPush...)
func (l *List) RawValue() interface{}   { return l.goList }
func (l *List) CreatedAt() time.Time    { return l.createdAt }
func (l *List) Frequency() int64        { return l.hits }
func (l *List) Category() string        { return "list_data" }
func (l *List) LastAccessed() time.Time { return time.Now() }

func (l *List) Update(newValue interface{}) error {
	// منطق التحديث للقوائم (مثلاً إضافة عنصر جديد تلقائياً)
	l.hits++
	return nil
}
func (l *List) OnDelete(key string, db interfaces.DatabaseOps) {
	duration := time.Since(l.createdAt)
	fmt.Printf("Key: %s | Lifetime: %v | Total Hits: %d\n", key, duration, l.hits)
}
...
func (l *List) AsString() (string, bool) {
	// القوائم لا تدعم التحويل المباشر لنص في Redis
	// ولكن يمكنك إرجاع عدد عناصرها كمعلومة تحليلية
	return fmt.Sprintf("List(length:%d)", l.LLen()), false
}
